...
label = ''
content = "Awainting fall detection"
persist_patient_id = ""

def fhir_resource_w19(patientId):
    """
    ...
    try:
        ...
        while True:            
            ...
            if len(data_processor.data_buffer) >= 30: # Collecting 30 data points for processing
                ...
                await websocket_manager.broadcast_message(json.dumps({'type' : "timeUpdate", 'content': f"Current label: {label}"}))

                # Remove first 10 data point from data buffer
                data_processor.sliding(10)
                print(datetime.now().strftime("%H:%M:%S.%f")[:-3], ':', label)
                    ...
                    if (label == 'falling') & (count_window == 0):
                        ...
                        if count_false >= 4:
                            ...
                            alert_message = json.dumps({
                                    'action':'updateDiv',
                                    'divId': 1,
                                    'content': str(fhir_resource_w19(persist_patient_id))
                                })
...
#         global label
#         now = label 
#         await websocket.send_text(f"Current label: {now}")
